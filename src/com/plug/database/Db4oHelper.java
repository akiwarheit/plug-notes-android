...
public class Db4oHelper {
  ...
  public ObjectContainer db() {
    try {
      if (oc == null || oc.ext().isClosed()) {
        oc = Db4oEmbedded.openFile(dbConfig(), context.getDir("databases", 0)
            + "/" + DATABASE_NAME);
      }
      return oc;
    } catch (Exception e) {
  ...
  private EmbeddedConfiguration dbConfig() throws IOException {
    ...
    configuration.common().objectClass(Note.class).objectField("title")
        .indexed(true);
    configuration.common().objectClass(Note.class).objectField("content")
        .indexed(true);
    configuration.common().objectClass(Notebook.class)
        .objectField("description").indexed(true);
