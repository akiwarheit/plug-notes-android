...
public class Note {
  ...
  @SerializedName("android_created_at")
  private String androidCreated;
  @SerializedName("android_updated_at")
  ...
  public static void store(Context context, Note note) {
    ...
    provider.store(note);
    provider.db().commit();
    // Log.i("Saving notebook", Note.find(context,
    // note).getNotebook().getDescription());
  }
  ...
  public static void update(Context context, Note note, String title,
      String content) {
    PlugApplication plug = (PlugApplication) context.getApplicationContext();
    NotesProvider provider = plug.getNotesProvider();
    Note currentNote;
  ...
  public static void update(Context context, Note note, String title,
      String content, Notebook notebook) {
    PlugApplication plug = (PlugApplication) context.getApplicationContext();
    NotesProvider provider = plug.getNotesProvider();
    Note currentNote;
    Date date = new Date(System.currentTimeMillis());
    ...
    Predicate<Note> constraint = new Predicate<Note>() {
      ...
      public boolean match(Note note) {
        ...
        if (!parameter.getNotebook().equals(null)) {
          if (note.getNotebook().equals(
              Notebook.find(finalContext, parameter.getNotebook())))
            if (note.getTitle().toLowerCase()
                .contains(parameter.getTitle().toLowerCase()))
              if (note.getUserId() == User.getLoggedInUser(finalContext)
                  .getId())
                return true;
        }
        if (parameter.getNotebook().equals(null)) {
  ...
  public static void upload(Context context, Note note)
